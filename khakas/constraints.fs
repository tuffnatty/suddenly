...
\ если после него также стоит любой аффикс, кроме ОК, или конец
\ словоформы. [Таким образом, после основы на невыпадающую
\ согласную в конце словоформы для NF возможны оба алломорфа.]
: constraint-4.1ₚ  ( -- f )
  ...
  OR OR ;
: constraint-4.1₀  ( -- f )
  2 form-slot-vowel-at-left? NOT  flag-Ass₁ flag-empty?  AND ;
